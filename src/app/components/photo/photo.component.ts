...
/* Soluion instead of using the angular default detection change we need to add it in this component. */
@Component({
  selector: 'app-photo',
  ...
  ngOnInit(): void {
    ...
    this.unsplashService.getPhoto(photoId).subscribe(photo => {
      ...
      if (photo && photo.response) {
        this.photo$.next(photo.response as unknown as IPhoto); // Adjust the type assertion as needed
      } else {
        console.error('Invalid photo response');
